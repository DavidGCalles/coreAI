...
## Tareas
- [X] Eliminar cualquier servicio acoplado a interfaces de usuario, bots o dependencias de lifeOS.
- [X] Reconfigurar el contenedor principal para que actúe estrictamente como `core-api` (Servidor MCP), eliminando variables de entorno obsoletas.
- [X] Validar y mantener los contenedores de infraestructura existentes (`db-relational`, `db-vector`, `llm-proxy`, broker de mensajería).
- [X] Añadir/Configurar el servicio `worker-node` para que consuma del broker de mensajería usando la misma imagen base que la API.
